...
**/	
	...
	function getVulnNodeTitle($func_name)
	{
		...
		{	$vulnname = $GLOBALS['NAME_OTHER']; } // :X			 			
			...
			$vulnname = "unknown";
        #echo '<pre>' . var_export("$func_name - $vulnname", TRUE) . '</pre>';die('<pre>Exit at Line '.__LINE__.' of <span title="'.__FILE__.'">'.str_replace(array(__DIR__, '\\'), '', __FILE__).'</span> @ '.date('H:i:s'));
		return $vulnname;	
	}
						...
						if($vulnBlock->vuln)	
						{
							...
							foreach($vulnBlock->treenodes as $tree)
							{
								...
								{	
									...
									// $tree->title
									echo '</td><td class="before-vulntitle"><span class="vulntitle">',$tree->title,'</span>',
