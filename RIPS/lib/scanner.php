<?php
session_start();
error_reporting(E_ALL ^ E_DEPRECATED ^ E_STRICT ^ E_WARNING ^ E_NOTICE);  //  E_ERROR, E_WARNING, E_PARSE, E_NOTICE, E_CORE_ERROR, E_CORE_WARNING, E_COMPILE_ERROR, E_COMPILE_WARNING, E_USER_ERROR, E_USER_WARNING, E_USER_NOTICE, E_STRICT, E_RECOVERABLE_ERROR, E_DEPRECATED, E_USER_DEPRECATED, E_ALL
/** 
...
class Scanner
{
	...
	function parse()
	{
					...
					{	
						if(!$this->already_scanned($i))
						{
							...
							if( ($has_vuln_parameters && ($parameter_has_userinput || $parameter_func_depend)) || $GLOBALS['verbosity'] == 4 || isset($this->scan_functions[$token_value][3]) ) 
							{
                                ...
                                $ignorename = str_replace($_SESSION['root'], '', $this->file_name);
        ...
        #echo '<pre>' . var_export($GLOBALS['output'], TRUE) . '</pre>';
