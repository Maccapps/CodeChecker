...
class PHP_CodeSniffer
{
    ...
    public function process($files, $standard, array $sniffs=array(), $local=false)
    {
        ...
        foreach ($GLOBALS['cfg']['sniffs'] as $this_sniff) {
            $path = explode('_', $this_sniff);
            $active = true;
            if (in_array($this_sniff, $GLOBALS['codeCheck']->cfg['excludes']['sniffs'])) {
                $active = false;
            }
            if ($active === false OR in_array($path[0], $GLOBALS['codeCheck']->cfg['excludes']['standards'])) {
                $active = false;
            }
            if ($active === false OR in_array($path[2], $GLOBALS['codeCheck']->cfg['excludes']['types'])) {
                $active = false;
            }
            if ($active) {
                $this->listeners[] = $this_sniff;
            }
        }
    ...
    public function setTokenListeners($standard, array $sniffs=array())
    {
        ...
        foreach ($files as $file) {
            ...
            $parentpath = substr($fullpath, 0, strlen($fullpath) - strlen($folder));
            ini_set('include_path', $parentpath);
     ...
     *                                   not exist.
     */
    public function getSniffFiles($dir)
    {
        $ownSniffs      = array();
     ...
     * Standards directory. Valid coding standards
    ...
    public static function setConfigData($key, $value, $temp=false)
    {
        if ($temp === false) {
            ...
            if (is_file($configFile) === false) {
                $configFile = '@data_dir@/PHP_CodeSniffer.conf';
            }
